...
class QRDQN(nn.Module):
    ...
    def reset_noise(self):
        ...
        self.noisy_values2.reset_noise()
...
def compute_td_loss(batch_size):
    ...
    hubber_loss = 0.5 * tau.abs().clamp(min=0.0, max=k).pow(2)
    hubber_loss += k * (tau.abs() - tau.abs().clamp(min=0.0, max=k))
    quantile_loss = (tau - (tau < 0).float()).abs() * hubber_loss
...
def plot(iteration, rewards, losses, ep_reward):
    print(f'Outputing Iteration {iteration}')
